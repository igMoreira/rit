...
public class Ark3 implements BlockCipher{
	...
	public byte[] execute(byte[] text) {// text is equals the IV for the first round or the cipherText for the next rounds
		...
		System.arraycopy(key,0,keyState,0,key.length);
//		this.keyState = Arrays.copyOf(key, key.length);
		return Ark3Cipher(text); 
	}
	...
	private byte[] generateSubkey()
	{
		...
		Packing.packLongBigEndian(keyState, 0, test, 0, 2);
		after[0] = test[0] >>> 29;
		after[0] |= test[1] << 35;
		after[1] = test[1] >>> 29;
		after[1] |= test[0] << 35;
		Packing.unpackLongBigEndian(after,0, keyState,0,2);
	...
	private byte[] permutation(byte[] state)
	{
		...
		for (int i = 0; i < state.length; i++) {
			byte b = (byte) 0xC0; // (x7 + x6) 11000000
			byte a = state[i];
			byte c = 0x63; // (x6 + x5 + x + 1)

			//  (x8 + x4 + x3 + x2 + 1). The formula for the S-box is Si(a) = ((x7 + x6) + i)×a + (x6 + x5 + x + 1)
			byte d = 0x00;
			d = (byte) (b ^ (i+1)); // ((x7 + x6) + i)
			d = (byte) multiplyGalois(d, a);
			d = (byte) (d ^ c);
			/*d = (multiplyGalois(a, b)^(multiplyGalois(a, i)));*/
			result[i] = d;
		}
